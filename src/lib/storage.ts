...
export async function downloadUrl(key:string){if(key.startsWith("http://")||key.startsWith("https://")) return key; return getSignedUrl(client(),new GetObjectCommand({Bucket:bucket(),Key:key}),{expiresIn:300});}
