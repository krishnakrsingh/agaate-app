 ...
 *
 * Response shaping, error mapping, pagination envelope, and sort allow-listing.
...
export function apiError(error: unknown) {
  ...
  if (
    ...
    (error.message === "Unauthenticated" || error.message === "Account is unavailable")
  ) {
  ...
  if (error instanceof Prisma.PrismaClientValidationError) {
    ...
    return NextResponse.json(
      {
        error:
          "Database schema is out of date. Run `npx prisma generate` and restart the dev server.",
      },
      { status: 500 }
    );
  }
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    ...
    return NextResponse.json(
      { error: "Database operation failed. Please check server logs." },
      { status: 500 }
    );
  }
  if (
    error instanceof Error &&
    /not configured|provider is temporarily unavailable/i.test(error.message)
  ) {
    console.error(error);
    return NextResponse.json(
      { error: "This integration is temporarily unavailable." },
      { status: 503 }
    );
  }
