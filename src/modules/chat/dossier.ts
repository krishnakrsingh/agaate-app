...
export async function getFarmDossier(farmId: string) {
  const [farm, plots, incidents, prescriptions, tasks, access] = await Promise.all([
    ...
    prisma.plot.findMany({
      ...
      select: {
        ...
        latitude: true, longitude: true,
        irrigation: { select: { type: true, details: true } },
        cropCycles: {
  ...
  return {
    ...
    plots: plots.map((p) => ({
      ...
      irrigationType: p.irrigation.map((i) => i.type).filter(Boolean).join(" · ") || null,
      irrigationDetails: p.irrigation.map((i) => i.details).filter(Boolean).join(" · ") || null,
      ...
      cycles: p.cropCycles.map((c) => ({
        id: c.id,
        cropName: c.cropName,
        variety: c.varieties.map((v) => v.name).filter(Boolean).join(", ") || null,
        status: c.status,
        startDate: c.startDate,
        expectedFirstHarvestDate: c.expectedFirstHarvestDate,
        actualPlants: c.actualPlants,
