...
export async function POST(_: Request, { params }: { params: Promise<{ farmId: string }> }) {
  try {
    ...
    if (!validActivationFrom.has(farm.status)) {
      return NextResponse.json(
        { error: `Farm activation is only allowed from SETUP status. Current status: ${farm.status}` },
        { status: 409 }
      );
    }
      ...
      farm.plots.some((p) =>
        p.cropCycles.some((c) => {
          const required = milestoneTemplates({
            ...
            establishmentType: c.establishmentType as any,
          }).map((m) => m.name);
