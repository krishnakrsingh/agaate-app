...
export async function POST(request: NextRequest, { params }: { params: Promise<{ taskId: string }> }) {
  try {
    ...
    if (input.actualPlants !== undefined && (!task.cropCycleId || !/(transplantation|direct sowing)/i.test(task.title))) return NextResponse.json({ error: "Actual plant count can only be recorded on a transplantation or sowing activity." }, { status: 422 });
