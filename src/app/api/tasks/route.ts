...
export async function GET(request: NextRequest) {
  try {
    const actor = await currentActor(); const search = request.nextUrl.searchParams; const farmId = search.get("farmId"); const day = search.get("date"); if (farmId) await requireFarmAccess(farmId);
    ...
    if (actor.role === "FARM_OFFICER" && !farmId) {
      const farms = await prisma.farm.findMany({ where: { status: "ACTIVE", access: { some: { userId: actor.id } } }, select: { id: true } });
      const cycles = await prisma.cropCycle.findMany({ where: { status: "ACTIVE", plot: { farmId: { in: farms.map(f => f.id) }, deletedAt: null } }, select: { id: true, cropName: true, plotId: true, plot: { select: { farmId: true } } } });
      for (const cycle of cycles) {
        const exists = await prisma.task.findFirst({ where: { origin: "DAILY_MONITORING", assignedOfficerId: actor.id, cropCycleId: cycle.id, dueDate: requestedDate } });
        if (!exists) await prisma.task.create({ data: { farmId: cycle.plot.farmId, plotId: cycle.plotId, cropCycleId: cycle.id, origin: "DAILY_MONITORING", category: "CROP_MONITORING", title: `Daily monitoring · ${cycle.cropName}`, description: "Record crop health, stage, remarks, and at least one photo.", dueDate: requestedDate, status: "ASSIGNED", assignedOfficerId: actor.id, createdById: actor.id } });
      }
    }
    ...
    const where = actor.role === "FARM_OFFICER" ? { assignedOfficerId: actor.id, ...(farmId ? { farmId } : {}), ...(day ? { dueDate: parseUtcDate(day) } : {}) } : { ...(farmId ? { farmId } : unrestricted ? {} : { farm: { access: { some: { userId: actor.id } } } }), ...(day ? { dueDate: parseUtcDate(day) } : {}) };
    return NextResponse.json(await prisma.task.findMany({ where, include: { farm: { select: { id: true, name: true } }, plot: { select: { name: true } }, cropCycle: { select: { cropName: true } }, assignedOfficer: { select: { name: true } }, executions: true }, orderBy: [{ dueDate: "asc" }, { priority: "desc" }], take: limit, skip: offset }));
